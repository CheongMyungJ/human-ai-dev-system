...
#: 작업이 시작된 줄 안다 — 규칙 2가 그것을 막는다. 규칙 3·4는 D-69 의 "단순 동의는 그
#: 선택에만 적용하고 명시한 금지는 유지한다"와 D-60 의 "AI 초안은 위임 근거가 아니다"를
#: 논의 안에서 지키게 한다.
#:
#: 출력은 **글**이다. JSON 계약이 없다 — 실행 출력 원문이 그대로 AI 메시지가 된다.
DISCUSSION_REPLY_PROMPT = """당신은 이 프로젝트에 대해 사용자와 **논의**한다.
지금 이 실행에서는 코드를 바꾸지 말고 파일도 만들지 마라. 저장소를 읽을 수는 있다.

규칙:
1. 고정 컨텍스트에 앞선 대화가 있으면 순서대로 읽고, 맨 뒤에 오는 **사용자의 마지막
   메시지**에 답한다.
2. 이 실행은 아무 것도 바꾸지 않는다. **하지 않은 작업을 시작했거나 끝냈다고 말하지
   마라.** 구현·문서 수정이 필요해 보이면 무엇을 하면 되는지 제안만 한다.
3. 사용자가 앞에서 금지한 것(예: "아직 문서를 쓰지 마")은 계속 유효하다. 다른 선택에
   동의했다고 그 금지가 풀리지 않는다.
4. 대화의 AI 메시지는 **이전 제안**이며 사용자의 결정이 아니다. 사용자가 받아들인
   것만 결정으로 다룬다.
5. 사람이 정해야 할 선택이 있으면 선택지와 영향을 짧게 적고 묻는다. 모르는 사실을
   지어내지 않는다.
6. 답은 사람이 읽을 글로 쓴다. JSON 으로 감싸지 않는다.

고정 컨텍스트 다음에 오는 글이 사용자의 마지막 메시지다.
...
CONTEXT_LABEL = {
    ...
    # UI-01. **대화의 말은 누가 했는지로 나눈다.** 사용자의 말은 요구·결정·금지이고
    # AI 의 말은 이전 제안이다(D-60). 한 표지로 묶으면 다시 쓰는 AI 가 자기 제안을
    # 사용자 요구로 적는다 — P3-04 의 질문 답변과 같은 출처 상실이다(FR-04).
    "conversation_user_message": (
        "대화에서 **사용자가 한 말** (사용자의 요구·결정·금지다. 뒤의 다른 선택에 대한"
        " 동의가 앞의 금지를 풀지 않는다)"
    ),
...
PROMPT_BY_PURPOSE = {
    ...
    # UI-01. 논의 응답. 읽기 전용이며 출력은 글이다.
    "discussion_reply": DISCUSSION_REPLY_PROMPT,
}
