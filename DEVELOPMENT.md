...
| 현재 단계 | **P1 — CLI 연결 검증** |
| 현재 단계 상태 | `WAITING_USER` — P1-03 성공 기준 중 '지연 이벤트·응답 유실에서 중복 호출하지 않음'이 미검증이다. P2 이월 수용 여부를 사용자가 정해야 P1을 닫을 수 있다 |
| 다음 하위 작업 | 없음. **P1-01~04 수행 완료**. 단계 완료 판정에 사용자 결정 1건 필요(아래 참조) |
| 활성 plan | `P1-PLAN-01` — 아래 [활성 plan](#활성-plan) 참조. 1~16번 모두 완료 |
...
| 필요한 사용자 결정 | **1건.** P1-03의 '중복 호출 방지' 검증을 P2-01(제어부·Runner 계약)로 이월하고 P1을 `ACCEPTED_WITH_LIMITATIONS`로 닫을지, 아니면 P1에서 최소 제어부를 만들어 마저 검증할지 |
| 다음 세션 첫 행동 | 시작 절차 → 위 사용자 결정이 되어 있는지 확인 → 결정에 따라 P1을 닫고 P2-01로 넘어가거나 P1-03 잔여를 수행 |
...
|---|---|---|---|
| P1 CLI 연결 검증 | WAITING_USER | 없음(결정 대기) | P1-01~04 수행 완료 — [계약](p1-environment-contract.md), [P1-02 결과](p1/evidence/P1-02-results.md), [P1-03 결과](p1/evidence/P1-03-results.md), [OpenCode 계약](p1/opencode/contract.md). 중복 호출 방지만 미검증 |
...
마지막 항목 때문에 P1을 `DONE`으로 표시하지 않는다. 선택지는 둘이며 **사용자 결정 사항**이다.

- (가) 이 검증을 P2-01(최소 골격·Runner 계약)으로 이월하고 P1을 `ACCEPTED_WITH_LIMITATIONS`로 닫는다.
  근거: `run_id` 멱등성과 배정 세대는 제어부 설계의 일부이며 P2-01 범위와 겹친다
- (나) P1에서 최소 제어부(요청 재전송·run_id 대조)를 만들어 마저 검증한 뒤 P1을 `DONE`으로 닫는다.
  근거: P1-03 성공 기준을 그대로 충족시킨다. 대신 P2-01 작업 일부를 앞당겨 하게 된다

결정 전까지 P2 구현을 시작하지 않는다.
...
단계 완료 여부와 이유:
  P1 미완료(WAITING_USER). P1-01~04를 모두 수행했고 완료 점검은 4절 P1 절에 표로 남겼다.
  막는 항목은 하나다 — P1-03 성공 기준의 '지연 이벤트·응답 유실에서 중복 호출하지 않음'.
  재배정·멱등 검사는 제어부가 있어야 확인 가능한데 P1 범위에 제어부가 없다.
  이월 수용 여부가 사용자 결정 사항이므로 임의로 DONE 처리하지 않았다
다음 단계·하위 작업 / 다음 세션 첫 명령 또는 읽을 위치:
  사용자 결정 확인이 먼저다. 4절 P1 절의 'P1 완료 점검' 표와 선택지 (가)/(나)를 읽는다.
  (가) 이월 수용 → P1을 ACCEPTED_WITH_LIMITATIONS로 닫고 P2-01 착수(새 plan 필요)
  (나) P1에서 마저 검증 → 최소 제어부(요청 재전송·run_id 대조)를 P1-PLAN-01에 추가해 수행
막힌 조건과 필요한 사용자 답변:
  P1 완료 판정이 막혀 있다. '중복 호출 방지' 검증을 P2로 이월할지((가)),
  P1에서 최소 제어부를 만들어 마저 검증할지((나)) 답이 필요하다. 4절 P1 절 참조
```
