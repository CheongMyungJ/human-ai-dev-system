...
상태: **수용 확인 끝(S-033) → 빠진 것 해소(S-034).** S-033 의 대조에서 네 축 중 셋(혼합 목적의 계약, 실패/복구/변경, Case 간 지식)은
근거가 완비돼 있었고, "여섯 Profile 대표" 는 계약·판정 수준에서 여섯 전부 재현됐으나 제품 흐름 수준(P4-05 진행기 경유)에서는
셋(feature·research·RCA)만 있었다. 빠진 것 — defect-fix·refactoring·maintenance 의 진행기 경유 대표 흐름과 혼합 목적의 진행기 경유
완료 — 는 [P4-PLAN-08](../../plans/P4-PLAN-08.md)로 기록했고, **사용자 결정(2026-09-24: "P4-PLAN-08 먼저 하고 닫아줘")으로 S-034 가
같은 대화에서 이어서 구현·검증했다** — [P4-08 결과](P4-08-results.md)(새 시험 13건, 제품 코드 무변경, 제품 결함 없음). 이로써 여섯
Profile 전부와 혼합 목적이 제품 흐름 수준에서도 재현됐다(아래 1.1·1.2절의 갱신 행). 남은 것은 실제 codex 의 한계(6절)뿐이며 P4 의
종료 상태는 DEVELOPMENT 4절.**
...
| defect-fix | `::test_a_defect_already_fixed_closes_without_any_code_change`(검증 실행 하나로 무변경 완료), `::test_already_satisfied_must_be_observed_by_a_verification_run`, `tests/test_progression.py::test_not_reproducing_a_defect_is_not_a_met_criterion`, `::test_an_already_satisfied_goal_needs_run_evidence`, 혼합 `test_profile_completion.py::test_a_fix_that_also_asked_for_the_cause_needs_both`, 잔여 `::test_experiment_residue_blocks_auto_completion_of_a_product_case_only`(결함 Case 자동 완료 거부) | S-033 시점: RCA 에서 개정된 defect_fix 만(`tests/test_profile_revision.py::test_a_revision_records_history_keeps_everything_else_and_carries_the_criteria` — 동의 대기까지, 브라우저 `::test_a_purpose_change_in_the_work_stage_revises_the_profile_and_reauthors_the_intent`). **S-034(P4-08) 추가**: `tests/test_profile_flows.py::test_a_defect_fix_restores_the_behavior_through_the_progressor`(복원 → `changed_and_verified` → 자동 완료), `::test_an_already_fixed_defect_closes_from_a_verification_run_alone`(검증 Task 만 → `already_satisfied`), `::test_a_failed_reproduction_records_nothing_and_waits`(exit 1 → 적지 않음) | P4-03 라이브 A(의도 작성 — `objectives: ["cause"]`·C-02 `restoration`), UI-01 라이브(defect-fix 초안) — 둘 다 의도 작성까지 | **재현됨**(계약 + 흐름, S-034) |
...
| refactoring | `::test_a_refactoring_without_a_preservation_criterion_does_not_close`, `::test_a_refactoring_closes_when_improvement_and_preservation_are_each_proven`(보존의 구현·실험 근거 거부, 검증 실행으로 완료) | S-033 시점 없음(`test_restart_recovery.py` 의 것은 Profile 값 보존만). **S-034(P4-08) 추가**: `tests/test_profile_flows.py::test_a_refactoring_closes_when_improvement_and_preservation_are_each_verified`(`improvement`/`changed_and_verified` + `preservation`/`preserved`), `::test_a_refactoring_without_a_preservation_criterion_waits_instead_of_closing`(`objective_without_criteria`), `::test_a_refactoring_whose_preservation_fails_keeps_the_verdict_and_waits` | P4-03 라이브 C(의도 작성 — 개선 + 보존 기준 둘) | **재현됨**(계약 + 흐름, S-034) |
| maintenance | `::test_maintenance_asks_for_preservation_only_when_conditions_were_written`(`required_by = profile_conditional`) | S-033 시점 없음. **S-034(P4-08) 추가**: `tests/test_profile_flows.py::test_maintenance_requires_preservation_only_when_conditions_are_written[conditions-both·conditions-target-only·no-conditions]`(`target_state` + 조건부 `preservation`/`preserved`, 조건 없으면 목표 상태만으로 닫힘) | **없음**(실제 codex 의 maintenance 의도 작성 표본 없음 — P4-08 AC-8 선택 사항, 미수행) | **재현됨**(계약 + 흐름, S-034; 실제 codex 관찰 없음) |
...
| (b) 흐름 | UI-04c AC-7(RCA → defect_fix 개정 → `intent_revision` → material delta → 동의 대기까지). **S-034(P4-08) 추가**: `tests/test_profile_flows.py::test_a_revised_case_closes_only_when_the_cause_and_the_fix_are_both_proven[both-proven·fix-only]`(동의 뒤 결합 기록·구현·검증 → 검증 실행이 `cause` 를 `investigated`·`determined` 로, `restoration` 을 `changed_and_verified` 로 적어야 닫힘; 수정만이면 `criteria_unresolved` 대기), `::test_a_defect_fix_that_declared_the_cause_from_the_start_needs_both[both-proven·fix-only]`(처음부터 선언한 `cause` 도 같은 규칙) | **재현됨**(S-034) |
...
| 6 | "원인 확정과 수정" 중 수정만 성공하면 완료 아님; 합의된 판단 불가는 정상 결과 가능 | `::test_a_fix_that_also_asked_for_the_cause_needs_both`, `::test_a_bounded_research_can_close_with_an_inconclusive_answer_and_no_code`, `::test_an_inconclusive_cause_is_not_success_when_the_cause_must_be_found` | 재현됨(계약 S-033 · 진행기 경유는 S-034 P4-08 — 1.2절) |
| 7 | 목표가 이미 충족됨을 유효한 증거로 확인하면 무변경 완료 가능, 미재현만으로 해결 선언 없음 | `::test_a_defect_already_fixed_closes_without_any_code_change`, `::test_already_satisfied_must_be_observed_by_a_verification_run`, `test_progression.py::test_not_reproducing_a_defect_is_not_a_met_criterion` | 재현됨(계약 S-033 · 진행기 경유는 S-034 P4-08 — 1.1절 defect-fix) |
...
| 10 | 미완료 RCA 에 같은 문제의 수정 요청 → 같은 Case 의 개정 연결, 기존 기준·증거·실패·repair·사용량 보존, 쓰기 조건 전 수정 실행 없음 | UI-04c AC-1·4·6·7·8, 브라우저 `test_a_purpose_change_in_the_work_stage_revises_the_profile_and_reauthors_the_intent` | 재현됨(동의까지 S-033; 그 뒤 완료는 S-034 P4-08 — 1.2절) |
...
| AC-07 Profile 변경·혼합 목적 | 검증됨(계약·개정 S-033, 진행기 경유 완료 S-034 P4-08) | P4-03 AC-10, UI-04c AC-1~8 — 1.2절 |
...
| AC-10 무변경 완료·미재현 금지 | 검증됨(계약 S-033, 진행기 경유 S-034 P4-08) | 2절 7행 |
| AC-11 불확실 결론의 완료 조건 | 검증됨 | 1.1절 RCA·research |
| AC-12 refactor/maintenance 의 개선·보존 입증 | 검증됨(계약 S-033, 진행기 경유 S-034 P4-08) | 1.1절 refactoring·maintenance |
...
## 4. 빠진 것 → P4-PLAN-08 (S-034 에 해소)

**S-034 갱신:** 아래 묶음은 [P4-08 결과](P4-08-results.md)로 해소됐다(새 시험 13건·제품 코드 무변경). 아래 문단은 S-033 시점의 판정 기록이다.
...
- **검증 실행이 이 호스트에서 시험을 돌리지 못한다** — 실제 codex 의 샌드박스 셸이 `python`·`rg` 를 찾지 못해 라이브에서는 기준이 전부
  ...
  **S-034 실험이 원인(샌드박스의 ACL)과 조건(인터프리터 폴더에 샌드박스 그룹 읽기·실행)을 찾았다 — [P4-ENV 결과](P4-ENV-results.md).** 제품 경유
  라이브는 아직이다.
...
1. **P4 의 종료 판정** — (가) P4-PLAN-08(진행기 경유 세 Profile + 혼합 목적 완료, 코드 변경 없이 시험 추가가 목표)을 먼저 하고 P4 를
   ...
   **사용자의 답(2026-09-24): (가) — S-034 가 P4-PLAN-08 을 마쳤다([P4-08 결과](P4-08-results.md)). 종료 상태는 DEVELOPMENT 4절.**
2. **검증 환경** — 실제 codex 가 검증 실행에서 시험을 돌릴 수 있게 하는 시도(절대 경로 인터프리터·PATH 전달)를 별도 작은 실험으로 할지.
   제품 기능이 아니라 환경·CLI 샌드박스의 문제이며, 성공하면 처음으로 실제 `met` 라이브 증거가 생긴다. **사용자의 답(2026-09-24): 해도
   된다(codex 사용 가능) — S-034 가 실험했다([P4-ENV 결과](P4-ENV-results.md)).**
3. **실제 codex 의 RCA·maintenance 관찰** — P4-PLAN-08 의 선택 사항(`p4/live/profile_intents.py` 재사용)으로 둘지. **사용자의 답(2026-09-24):
   뜻을 설명해 달라 — S-034 보고에 적었고 판단을 기다린다(미수행).**
