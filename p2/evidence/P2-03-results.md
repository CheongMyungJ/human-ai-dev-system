...
| 브라우저 화면의 나머지 경로 | 게이트 검토 실행·동의·제한 작업을 **화면에서** 누르는 경로는 확인하지 않았다. API로는 확인했다 | P2-04 |
...
7절을 쓴 뒤 화면을 실제 브라우저(Edge headless)로 눌러 봤고 **결함을 하나 찾아 고쳤다.**
기록은 [P2-03-ui-check.log](P2-03-ui-check.log)와 화면 캡처 두 장에 있다.

### 찾은 결함: 화면의 "AI에게 초안 요청"이 아무 일도 하지 않았다

`web/src/IntentPanel.tsx` 가 `createRun` 에 `tool_id` 를 넘기지 않아 서버 기본값인
`local-echo`(P2-01 골격 실행기)로 배정됐다. 그 실행기는 **정상 종료하면서 아무 것도
쓰지 않는다.** 화면에는 "요청했다"고 뜨고 의도 버전은 영영 생기지 않는다.

자동 시험이 이것을 잡지 못한 이유는 시험이 목적에 맞는 도구를 항상 명시했기 때문이다.
빠뜨린 쪽을 시험하지 않았다.

**화면만 고치지 않았다.** 같은 실수는 API 직접 호출로 돌아온다. 그래서 도구가 스스로
`coding_cli` 능력을 보고하고(실제 CLI는 `verified`, 골격 실행기는 `unsupported`)
진입 검사가 그것을 본다 — 초안 작성·의미 검토에 코딩 CLI가 아닌 도구를 배정하면
`tool_is_not_a_coding_cli` 로 거부된다. 화면은 고를 수 있는 도구만 보여 주고,
없으면 그 사실을 표시한다. 회귀 시험 2건을 넣었다
(`test_admission.py` 의 `test_the_skeleton_executor_cannot_author_or_review_an_intent`,
`test_the_skeleton_executor_is_still_fine_for_plain_analysis`).

### 화면으로 확인한 것

| 확인 | 결과 |
|---|---|
| 기능 Case를 열면 의도 화면과 QG-01 패널이 뜬다 | 확인 |
| `AI에게 초안 요청` 의 도구 목록 | `claude (print)`, `codex (exec)` — **골격 실행기는 없다** |
| 조건 없이 `실행 요청` | `진입 조건 미충족 — Run을 만들지 않았다` 와 사유 2건이 화면에 표시 ([캡처](P2-03-ui-refusal.png)) |
| **화면에서 누른 초안 요청** | 실제 Claude Code 2.1.278 이 실행돼 의도 v1 생성(`draft-1789911261498`) |
| 작성 주체 표시 | `작성 주체: AI가 작성해 제시한 초안 · 작성 실행 draft-…` ([캡처](P2-03-ui-ai-draft.png)) |
| 게이트 패널 | `종합 판정 검토 안 함 / 규칙 검사 통과 / AI 의미 검토 검토 안 함` 과 "규칙만 통과한 것은 통과가 아니다" 안내 |
| 발견 사항 표시 | 규칙의 권고 발견(`open_intent_questions_present`)이 필수/권고·확정/의심 표시와 함께 나온다 |
| 진입 검사 기록 표 | 허용 1건·거부 2건이 사유와 함께 표시 |
| 브라우저 콘솔 | 오류 2건 — `/favicon.ico` 404와 **일부러 만든 409 거부**뿐이다. 제품 오류 아님 |

**화면에서 누르지 않은 것:** 게이트 의미 검토 실행, 원문 열람·동의, 제한 작업 실행은
API로만 확인했다(4~6절). P2-04 화면 작업과 함께 누른다.
...
| **화면의 "AI에게 초안 요청"이 도구를 지정하지 않아 골격 실행기로 배정됐다** | 화면을 고치고, **같은 실수가 API로 돌아오지 못하도록 서버에서도 막았다**(`coding_cli` 능력 + `tool_is_not_a_coding_cli` 거부). 8-1절 |
| 화면 부제가 `P2-02 의도·피드백` 으로 남아 있었다 | `P2-03 진입 제어·제한 실행` 으로 고쳤다 |
