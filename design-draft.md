...
| 단계 | 검토 가능한 결과 | 통과 조건 |
|---|---|---|
| 1. CLI·Windows 연결 검증 | 각 어댑터의 구조화 이벤트·정책·별도 세션·중지·재개 실험 | 정확한 버전/모드별 증거. OpenCode는 문서 계약과 실증 미완료 분리 |
| 2. 로컬 수직 구현 | 요청 → 의도 초안·피드백·동의 → 설계/계획 → 구현·검증·사람 인수 | 저장·재시작과 구현 선행 조건을 실제 우회 없이 확인 |
| 3. 적응형 업무·검증 | 버그·원인 분석·연구·중간 수정·게이트 설정·2회 수정 | 목표별 종료조건, 질문 이월, 오래된 증거 배제와 예외 종료 |
| 4. GitHub 연결 | 새 기록 이슈·요약 append, 확인받은 push/PR, 리뷰·CI 읽기 | 게시 장애·응답 유실·대상 변경에 의한 잘못된 반영 방지 |
| 5. 원격 Runner·복구 | PC 원문 보관, 일시중계, 단절·재접속·백업/복원 | 서버에 원문이 남지 않고 안전 중지·상태 대조·원본 누락 표시 |
| 6. 전체 수용시험 | 세 배포 형태와 목표 운영 규모, 문서화·설치 스크립트 | 요구사항별 시험 증거와 미지원·미검증 범위 공개 |
