...
[DEVELOPMENT.md](DEVELOPMENT.md)와 [대화 중심 UI 설계](ui-conversation-design.md), [UI-02 결과](ui/evidence/UI-02-results.md)를 먼저 읽는다. **UI-02(입력·실행 제어)는 완료**했다 — CLI 가 Windows job 안에서 돌고 중단하면 손자까지 끝낸 뒤 활성 0 을 확인하며, Runner 가 죽으면 OS 가 트리를 끝내고, 재기동한 Runner 가 CLI 를 다시 부르지 않고 원장으로 대조한다. 확인 근거가 있을 때만 요청이 `interrupted` 로 풀리고 PC 미연결이면 입력을 받지 않는다(실제 codex 로 실증). **P4-04(문맥·재개)도 완료**했다 — 핵심/보조 등급과 실행당 인라인 한도(보조만 드러내어 생략, 핵심 초과·핵심 원문 미확인은 보류), Runner가 CLI 호출 전에 해시를 대조해 보고하는 영수증, 시작하지 않은 실행의 소비 0, 결과 시점 최신성, 재시작 뒤 원시 출력의 사용량 복구. 다음 작업은 **UI-03 — 기본 대화 화면**이며 새 `UI-PLAN-03`을 기록·공유한 뒤 시작한다. 완료된 P4-01~04·UI-01~02를 다시 열지 않는다. UI-01은 `a509063`, P4-04는 `5580896`으로 각각 사용자 지시에 따라 커밋·push했고, **UI-02는 이 인계 시점에 커밋하지 않았다**(작업 트리에 있다). 완료된 작업의 설명·시작점·이전 인계는 [보존본](development-history-v0.8.md)으로 옮겨 DEVELOPMENT.md를 줄였다.
