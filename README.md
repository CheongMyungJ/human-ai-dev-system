...
정본 위치는 `C:\git\human-ai-dev-system-design`이다. 이 디렉터리의 문서가 이후 설계·구현의 기준이다. 주요 제품 결정 58건을 통합했다. 구현은 **P2-03 진입 제어·제한 실행까지** 와 있다(아래 실행 방법 참조). 설치된 코딩 CLI(Codex·Claude Code)가 제품 경로로 연결되어 AI가 의도 초안을 쓰고 별도 세션이 그것을 QG-01로 검토하며, 실행 배정 전에 서버가 진입 조건을 검사한다. 결과 열람·사람 최종 확인, 나머지 품질 게이트, GitHub 연동, 원격·성능 시험은 아직이다.

## 개발을 시작하는 새 세션

[개발 진행 안내서 DEVELOPMENT.md](DEVELOPMENT.md)를 읽고 현재 단계를 수행한다. 이 문서에 현재 상태, 6단계별 범위·성공 기준, 코드 변경 전 필수 plan, 검증·인계 절차가 있다. P1은 제한 수용으로 종료했고 현재 단계는 P2 최소 업무 흐름이다(P2-01~03 완료, P2-04 남음).
...
여섯 항목 의도 초안(**AI 작성 또는 사람 작성**)·원문 열람·피드백·미정 질문·명시 동의,
...
**아직 아닌 것:** 결과·근거 열람 화면과 사람 최종 확인(P2-04), 코드를 바꾸는 실행과
설계·계획 검토(P3), 나머지 품질 게이트와 게이트 ON/OFF 설정(P4), GitHub(P5), 원격·인증(P6).
