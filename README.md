...
[DEVELOPMENT.md](DEVELOPMENT.md)와 [대화 중심 UI 설계](ui-conversation-design.md), [UI-03 결과](ui/evidence/UI-03-results.md)를 먼저 읽는다. **UI-03(기본 대화 화면)은 완료**했다 — 새 대화에서 메시지를 보내면 제어부가 읽기 전용 AI 응답을 만들어 붙이고 요청을 끝내며, 명확한 작업 요청은 AI 해석으로 같은 대화에서 업무로 전환된다(위임 근거는 사용자 메시지). 초안은 이 브라우저에 남고 접수가 확인된 것만 비워지며 자동으로 보내지지 않는다. 결과물은 읽던 버전이 고정된다(실제 codex·실제 Edge 로 확인). 구현 중 제어부 공유 연결의 동시 사용 결함(P2 부터)을 찾아 고쳤다. **UI-02(입력·실행 제어)도 완료**했다 — CLI 가 Windows job 안에서 돌고 중단하면 손자까지 끝낸 뒤 활성 0 을 확인하며, Runner 가 죽으면 OS 가 트리를 끝내고, 재기동한 Runner 가 CLI 를 다시 부르지 않고 원장으로 대조한다. 확인 근거가 있을 때만 요청이 `interrupted` 로 풀리고 PC 미연결이면 입력을 받지 않는다(실제 codex 로 실증). **P4-04(문맥·재개)도 완료**했다 — 핵심/보조 등급과 실행당 인라인 한도(보조만 드러내어 생략, 핵심 초과·핵심 원문 미확인은 보류), Runner가 CLI 호출 전에 해시를 대조해 보고하는 영수증, 시작하지 않은 실행의 소비 0, 결과 시점 최신성, 재시작 뒤 원시 출력의 사용량 복구. 다음 작업은 **P4-05 — 업무 단계 자동 진행 + 완료·예외·후속**이며 새 `P4-PLAN-05`를 기록·공유한 뒤 시작한다. 업무 단계 자동 진행을 P4-05 에 넣은 것은 사용자 결정(2026-09-23)이다(DEVELOPMENT.md 1.6절). 완료된 P4-01~04·UI-01~03을 다시 열지 않는다. UI-01은 `a509063`, P4-04는 `5580896`으로 각각 사용자 지시에 따라 커밋·push했고, UI-02는 `592eb39`로 사용자 지시에 따라 커밋·push했다. **UI-03은 커밋하지 않았다**(작업 트리, 사용자 지시 대기). 완료된 작업의 설명·시작점·이전 인계는 [보존본](development-history-v0.8.md)으로 옮겨 DEVELOPMENT.md를 줄였다.
