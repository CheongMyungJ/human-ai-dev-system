...
정본 위치는 `C:\git\human-ai-dev-system-design`이다. 이 디렉터리의 문서가 이후 설계·구현의 기준이다. 주요 제품 결정 58건을 통합했다. 구현은 **P2-02 의도·피드백까지** 와 있다(아래 실행 방법 참조). 실제 코딩 CLI 연결, 진입 조건 검사, 품질 게이트, GitHub 연동, 원격·성능 시험은 아직이다.
...
# 4) 전체 시험 (제품 pytest 39건 + P1 OpenCode 문서 계약 unittest 18건)
...
여섯 항목 의도 초안·원문 열람·피드백·미정 질문·명시 동의, Run 생성·배정·실행·결과 표시,
강제 종료 후 복원, 같은 요청 재전송의 중복 실행 방지.
**아직 아닌 것:** 진입 조건 검사와 실제 코딩 CLI 실행(P2-03), 결과 열람·사람 최종 확인(P2-04),
품질 게이트(P4), GitHub(P5), 원격(P6). 현재 Runner 실행기는 코딩 CLI가 아니다.
