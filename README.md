...
정본 위치는 `C:\git\human-ai-dev-system-design`이다. **현재 구현은 P4-04 완료·스키마 v17**(UI-01·P4-03의 Profile 정의 v2 위)다. 대화 중심 UI의 주요 제품 정책과 개발 순서를 문서에 2차 반영했고, UI-01이 대화·요청의 영속 기반과 서버 전송 잠금을, **P4-04가 고정 문맥의 등급·인라인 한도·실제 읽음 영수증·핵심 누락 보류·Runner 재시작의 사용량 복구를 구현했다.** 새 기본 대화 화면·입력/실행 제어·중단은 미구현이다. 화면 폭·간격·단축키와 기술 상세는 구현 과정에서 구체화한다. 화면 시안이나 실제 사용성 검증을 마쳤다는 뜻은 아니다.

## 다음 개발 세션

[DEVELOPMENT.md](DEVELOPMENT.md)와 [대화 중심 UI 설계](ui-conversation-design.md), [P4-04 결과](p4/evidence/P4-04-results.md)를 먼저 읽는다. **P4-04(문맥·재개)는 완료**했다 — 핵심/보조 등급과 실행당 인라인 한도(보조만 드러내어 생략, 핵심 초과·핵심 원문 미확인은 보류), Runner가 CLI 호출 전에 해시를 대조해 보고하는 영수증, 시작하지 않은 실행의 소비 0, 결과 시점 최신성, 재시작 뒤 원시 출력의 사용량 복구. 다음 작업은 **UI-02 — 입력·실행 제어**(Runner 수신/실행 분리·질문·중단·재연결)이며 새 `UI-PLAN-02`를 기록·공유한 뒤 시작한다. 완료된 P4-01~04·UI-01을 다시 열지 않는다. UI-01은 `a509063`, P4-04는 `5580896`으로 각각 사용자 지시에 따라 커밋·push했다. 완료된 작업의 설명·시작점·이전 인계는 [보존본](development-history-v0.8.md)으로 옮겨 DEVELOPMENT.md를 줄였다.

> DEVELOPMENT.md의 최신 인계(S-021)와 P4-04 결과를 읽고 UI-02(입력·실행 제어)를 진행해줘. P4-04·UI-01·P4-03 완료 상태를 보존하고 새 UI-PLAN-02를 기록·공유한 뒤, 해당 범위의 구현·검증·인계 절차를 지켜줘. UI-03 기본 화면과 UI 전체 개편으로 범위를 확대하지 말아줘.

P3은 제한을 명시한 완료 상태이고 P4-01~04·UI-01도 완료됐다. 개발 순서는 **UI-01(완료) → P4-04(완료) + UI-02 → UI-03 기본 화면 → P4-05~07 + UI-04 추가 흐름 → P5/P6**다. P4-04 문맥·재개와 UI-02의 수신/실행 분리·질문·중단·복구는 별도 plan으로 다루며, 중단은 실제 확인 능력을 검증한 뒤 화면에 연결한다. UI-04는 착수 시 세분 plan으로 나누고 P4-04에 전체 UI를 몰아넣지 않는다.
