...
정본 위치는 `C:\git\human-ai-dev-system-design`이다. **현재 구현은 P4-03 완료·스키마 v15·Profile 정의 v2**다. 대화 중심 UI의 주요 제품 정책과 개발 순서를 문서에 2차 반영했으며 **새 UI와 관련 상태·제어 경로는 미구현**이다. 화면 폭·간격·단축키와 기술 상세는 구현 과정에서 구체화한다. 화면 시안이나 실제 사용성 검증을 마쳤다는 뜻은 아니다.

## 다음 개발 세션

[DEVELOPMENT.md](DEVELOPMENT.md)와 [대화 중심 UI 설계](ui-conversation-design.md)를 먼저 읽는다. 다음 작업은 **UI-01 — 대화·요청 기반**이며 개발 세션에서 새 `UI-PLAN-01`을 기록·공유한 뒤 시작한다. 준비 Case, 메시지·정정·참조, 요청 처리 상태와 API 전송 잠금, 접수 멱등성, 최초 업무화를 다룬다. 실제 중단 제어는 UI-02다. 이번 문서 반영으로 구현에 착수하지 않으며 완료된 P4-01~03을 다시 열지 않는다.

> DEVELOPMENT.md와 ui-conversation-design.md의 최신 인계를 읽고 UI-01(대화·요청 기반)을 진행해줘. P4-03 완료 상태와 기존 변경을 보존하고 새 UI-PLAN-01을 기록·공유한 뒤, 해당 범위의 구현·검증·인계 절차를 지켜줘. 실제 중단 제어와 UI 전체 개편으로 범위를 확대하지 말아줘.

P3은 제한을 명시한 완료 상태이고 P4-01~03도 완료됐다. 개발 순서는 **UI-01 → P4-04 + UI-02 → UI-03 기본 화면 → P4-05~07 + UI-04 추가 흐름 → P5/P6**다. P4-04 문맥·재개와 UI-02의 수신/실행 분리·질문·중단·복구는 별도 plan으로 다루며, 중단은 실제 확인 능력을 검증한 뒤 화면에 연결한다. UI-04는 착수 시 세분 plan으로 나누고 P4-04에 전체 UI를 몰아넣지 않는다.
